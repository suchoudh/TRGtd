...
//      System.out.println("======> OUTPUT HEADINGS: " + outputHeadings.getValue() + " <======");
        if (Boolean.parseBoolean(outputHeadings.getValue())) {
            for (Param param : params) {
                if (param.id.startsWith("field") && !param.getValue().equals("")) {
//                  System.out.println("======> ADDING PARAM - ID: h-" + param.id + ", VALUE: " + param.getValue() + " <======");
                    headingParams.add(new Param("h-" + param.id, getString(param.getValue())));                    
                }
            }
        }
